...
function StatusPill({ children, tone = "neutral" }: { children: ReactNode; tone?: "success" | "danger" | "warning" | "neutral" }) {
  return <span className={cn("inline-flex min-h-8 items-center rounded-full px-3 text-base font-bold", tone === "success" && "bg-[#DDF8E9] text-[#0F4D3A]", tone === "danger" && "bg-[#FFF0D9] text-[#A33D10]", tone === "warning" && "bg-[#FFF0D9] text-[#93540A]", tone === "neutral" && "bg-[#ECE9E3] text-[#595650]")}>{children}</span>;
}
...
export function StaffMobileRedesignWorkspace({ initialBundle, restaurantId, restaurantName, restaurantSlug, userId, enableHeartbeat = true }: StaffMobileRedesignWorkspaceProps) {
  ...
  if (!staff) {
    return (
      <main className="staff-brand-page staff-brand-page--mobile grid place-items-center p-5 text-[#2B2B2B]">
  ...
  return (
    <main className="staff-brand-page staff-brand-page--mobile text-[#2B2B2B]">
      ...
      <section className="mx-auto grid w-full max-w-6xl gap-5 px-5 pb-[calc(7.5rem+env(safe-area-inset-bottom))] pt-7 lg:grid-cols-[minmax(390px,460px)_minmax(0,1fr)] lg:pb-10">
        ...
        <aside className="hidden min-w-0 space-y-5 lg:block">
          <AppCard className="p-6"><p className="text-sm font-black uppercase tracking-[0.12em] text-[#5E5A54]">Đang hoạt động</p><h2 className="mt-2 text-3xl font-black text-[#2B2B2B]">{staff.fullName}</h2><p className="mt-1 text-base font-semibold text-[#5E5A54]">{staff.roleTitle} · {selectedBranchName}</p><div className="mt-5 grid grid-cols-3 gap-3"><MiniStat label="Ca" value={todayAssignments.length || upcomingAssignments.length} /><MiniStat label="Việc" value={workItems.length} /><MiniStat label="Tin" value={bundle.unreadNotificationCount} /></div></AppCard>
          <AppCard className="p-6"><p className="text-sm font-black uppercase tracking-[0.12em] text-[#5E5A54]">Chấm công</p><div className="mt-4 flex items-center justify-between"><div><p className="text-2xl font-black text-[#2B2B2B]">{activeAttendance ? "Đang trong ca" : "Sẵn sàng"}</p><p className="mt-1 text-sm font-semibold text-[#5E5A54]">{activeAttendance ? activeDuration : selectedBranchName}</p></div><StatusPill tone={activeAttendance ? "success" : "neutral"}>{mounted && offlineQueue.isOnline ? "Online" : "Offline"}</StatusPill></div></AppCard>
        </aside>
      </section>
...
function MiniStat({ label, value }: { label: string; value: ReactNode }) {
  return <div className="rounded-xl bg-[#F5F8F1] p-3"><p className="text-xs font-black uppercase tracking-[0.08em] text-[#5E5A54]">{label}</p><p className="mt-1 text-2xl font-black text-[#2B2B2B]">{value}</p></div>;
}
...
function HomeTab({ staffName, workItems, processingKey, onRunWorkItem, currentShift, activeDuration }: { staffName: string; workItems: StaffOpsMobileWorkItem[]; processingKey: string | null; onRunWorkItem: (item: StaffOpsMobileWorkItem) => void; currentShift: StaffOpsShiftAssignment | null; activeDuration: string | null }) {
  return (
    <div className="space-y-5">
      <section><h1 className="text-[40px] font-black leading-tight text-[#2B2B2B]">Hôm nay</h1><p className="mt-2 text-xl font-medium text-[#3F3D39]">{staffName}</p></section>
      <AppCard className="p-5"><div className="flex items-center justify-between"><div><p className="text-sm font-black uppercase tracking-[0.12em] text-[#5E5A54]">Ca hiện tại</p><h2 className="mt-2 text-2xl font-black text-[#2B2B2B]">{currentShift?.shiftName ?? "Chưa có ca"}</h2><p className="mt-1 text-base font-semibold text-[#5E5A54]">{currentShift?.branchName ?? "Theo phân công"}</p></div><StatusPill tone={activeDuration ? "success" : "neutral"}>{activeDuration ?? "Chờ ca"}</StatusPill></div></AppCard>
      <section className="space-y-3"><h2 className="text-sm font-black uppercase tracking-[0.12em] text-[#5E5A54]">Việc trong ca</h2>{workItems.length ? workItems.slice(0, 6).map((item) => <AppCard key={item.id} className="p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-xl font-black text-[#2B2B2B]">{item.title}</p><p className="mt-1 text-base font-medium text-[#5E5A54]">{item.subtitle}</p></div><StatusPill tone={item.priority === "high" ? "danger" : item.priority === "medium" ? "warning" : "neutral"}>{item.priority === "high" ? "Gấp" : item.priority === "medium" ? "Vừa" : "Thấp"}</StatusPill></div>{item.action ? <ShellButton onClick={() => onRunWorkItem(item)} disabled={processingKey === item.id} className="mt-4 w-full">{processingKey === item.id ? "Đang xử lý..." : item.actionLabel ?? "Xử lý"}</ShellButton> : null}</AppCard>) : <AppCard className="grid min-h-36 place-items-center p-5 text-center"><div><Check className="mx-auto text-[#0F4D3A]" /><p className="mt-2 text-lg font-black">Không có việc đang chờ</p></div></AppCard>}</section>
    </div>
  );
}
...
function ScheduleTab({ assignments, branchName }: { assignments: StaffOpsShiftAssignment[]; branchName: string }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-5 gap-3">{days.map((day, index) => <button key={day.iso} className={cn("grid h-24 place-items-center rounded-2xl border text-center", index === 0 ? "border-[#0F4D3A] bg-[#0F4D3A] text-white" : "border-[#E5DDD2] bg-white text-[#2B2B2B]")}><span className="text-base font-bold">{day.label}</span><span className="text-2xl font-medium">{day.day}</span></button>)}</div>
...
function ShiftGroup({ title, assignments }: { title: string; assignments: StaffOpsShiftAssignment[] }) {
  return <section className="space-y-4"><div className="flex items-center gap-3"><span className="text-3xl text-[#F28C28]">☼</span><h2 className="text-lg font-black uppercase tracking-[0.08em] text-[#3F3D39]">{title}</h2><span className="h-px flex-1 bg-[#E5DDD2]" /></div>{assignments.map((assignment) => <AppCard key={assignment.id} className="p-5"><div className="flex items-center justify-between"><div><h3 className="text-2xl font-medium text-[#2B2B2B]">{assignment.shiftName}</h3><p className="mt-2 text-xl font-medium text-[#5E5A54]">{assignment.branchName ?? "Chi nhánh"}</p></div><StatusPill tone="success">Đã gán</StatusPill></div><div className="mt-5 border-t border-[#E5DDD2] pt-5"><p className="text-xl font-medium text-[#2B2B2B]">{assignment.staffName}</p></div></AppCard>)}</section>;
}
...
function AttendanceTab({ staffName, activeAttendance, latestAttendance, selectedBranchId, selectedBranchName, branches, onBranchChange, onClock, processing, gpsEnabled, qrReady, online, queueLength, syncing, onSync, activeDuration, onTimeCount, lateCount }: { staffName: string; activeAttendance: ReturnType<typeof activeAttendanceForMember>; latestAttendance: StaffOperationsBundle["attendanceFeed"][number] | null; selectedBranchId: string; selectedBranchName: string; branches: StaffOperationsBundle["branches"]; onBranchChange: (value: string) => void; onClock: (source: ClockSource) => void; processing: boolean; gpsEnabled: boolean; qrReady: boolean; online: boolean; queueLength: number; syncing: boolean; onSync: () => void; activeDuration: string; onTimeCount: number; lateCount: number }) {
  return (
    <div className="space-y-6">
      <section><h1 className="text-[38px] font-black leading-tight text-[#2B2B2B]">Chấm công hôm nay</h1><p className="mt-2 text-xl font-medium text-[#3F3D39]">{formatDate(todayInputValue())}</p></section>
      <div className="grid grid-cols-2 gap-4"><AppCard className="p-5"><p className="flex items-center gap-2 text-lg font-black text-[#0F4D3A]"><Check size={24} /> Đúng giờ</p><p className="mt-6 text-[42px] font-black">{onTimeCount}</p></AppCard><AppCard className="p-5"><p className="flex items-center gap-2 text-lg font-black text-[#A33D10]"><Clock3 size={24} /> Đi muộn</p><p className="mt-6 text-[42px] font-black">{lateCount}</p></AppCard></div>
      <AppCard className="p-5"><div className="flex items-center justify-between gap-4"><div className="flex items-center gap-4"><span className="grid h-20 w-20 place-items-center rounded-full bg-[#E8E5E0] text-2xl font-black text-[#0F4D3A]">{initials(staffName)}</span><div><p className="text-[30px] font-black leading-tight text-[#2B2B2B]">{activeAttendance ? "Đang trong ca" : "Sẵn sàng"}</p><p className="text-xl font-medium text-[#3F3D39]">{selectedBranchName}</p></div></div><StatusPill tone={activeAttendance ? "success" : "neutral"}>{activeAttendance ? activeDuration : "Chưa vào"}</StatusPill></div><div className="mt-6 grid grid-cols-2 divide-x divide-[#D8D1C7] rounded-xl bg-[#F5F8F1] p-4 text-xl font-black"><span>{shortTime(activeAttendance?.clockInAt ?? latestAttendance?.clockInAt)}</span><span className="pl-4">{shortTime(activeAttendance?.clockOutAt ?? latestAttendance?.clockOutAt)}</span></div><label className="mt-5 block"><span className="sr-only">Chi nhánh</span><select value={selectedBranchId} onChange={(event) => onBranchChange(event.target.value)} className="h-14 w-full rounded-xl border border-[#D8D1C7] bg-white px-4 text-base font-bold">{branches.map((branch) => <option key={branch.id} value={branch.id}>{branch.name}</option>)}</select></label><div className="mt-4 grid gap-3"><ShellButton disabled={processing || !gpsEnabled} onClick={() => onClock("gps")} className="min-h-16"><MapPin size={22} /> {gpsEnabled ? activeAttendance ? "Kết ca bằng GPS" : "Check-in bằng GPS" : "GPS chưa bật"}</ShellButton><div className="grid grid-cols-2 gap-3"><ShellButton variant="secondary" disabled={!online || processing} onClick={() => onClock("wifi")}><Wifi size={21} /> WiFi</ShellButton><ShellButton variant="secondary" disabled={!qrReady || processing} onClick={() => onClock("qr")}><Fingerprint size={21} /> QR</ShellButton></div>{queueLength ? <ShellButton variant="ghost" onClick={onSync} disabled={syncing}><RefreshCw size={18} className={syncing ? "animate-spin" : undefined} /> Đồng bộ {queueLength}</ShellButton> : null}<p className="flex items-center gap-2 text-sm font-bold text-[#5E5A54]">{online ? <Wifi size={16} /> : <WifiOff size={16} />} {online ? "Thiết bị online" : "Thiết bị offline"}</p></div></AppCard>
    </div>
  );
}
...
function ReportsTab({ bundle, staffId }: { bundle: StaffOperationsBundle; staffId: string }) {
  ...
  return <div className="space-y-6"><section><h1 className="text-[38px] font-black text-[#2B2B2B]">Báo cáo</h1><p className="mt-2 text-xl font-medium text-[#3F3D39]">Tổng quan công, ca và hiệu suất cá nhân.</p></section><div className="grid grid-cols-2 gap-4"><AppCard className="p-5"><p className="text-sm font-black uppercase tracking-[0.12em] text-[#5E5A54]">Số ca</p><p className="mt-4 text-[42px] font-black">{attendanceCount}</p></AppCard><AppCard className="p-5"><p className="text-sm font-black uppercase tracking-[0.12em] text-[#5E5A54]">Điểm công</p><p className="mt-4 text-[42px] font-black">{score}</p></AppCard><AppCard className="p-5"><p className="text-sm font-black uppercase tracking-[0.12em] text-[#5E5A54]">Tăng ca</p><p className="mt-4 text-[42px] font-black">{Math.round(overtime / 60)}h</p></AppCard><AppCard className="p-5"><p className="text-sm font-black uppercase tracking-[0.12em] text-[#5E5A54]">Yêu cầu</p><p className="mt-4 text-[42px] font-black">{bundle.approvals.length}</p></AppCard></div><AppCard className="p-6"><h2 className="text-2xl font-black">Giờ công gần đây</h2>{attendanceRows.length ? <div className="mt-6 flex h-52 items-end gap-3 border-b border-[#D8D1C7]">{attendanceRows.map((item) => { const minutes = attendanceWorkMinutes(item); return <span key={item.id} className="flex flex-1 flex-col items-center justify-end gap-2"><span className="w-full rounded-t-xl bg-[#0F4D3A]" style={{ height: `${Math.max(8, Math.round((minutes / maxMinutes) * 100))}%` }} /><span className="text-[11px] font-black text-[#5E5A54]">{new Date(item.clockInAt).getDate()}</span></span>; })}</div> : <InlineEmptyState title="Chưa có log công" text="Biểu đồ chỉ hiển thị khi có chấm công thật." />}</AppCard></div>;
}
