...
export async function createStaffDevice({
  ...
  if (input.trustedForAttendance && !input.deviceFingerprint) {
  ...
  const result = await supabase
    ...
    .insert({
      ...
      trusted_for_attendance: Boolean(input.trustedForAttendance),
      trusted_at: input.trustedForAttendance ? new Date().toISOString() : null,
      issued_at: input.issuedAt,
      status: "assigned",
      note: input.note || null,
      created_by: actorUserId
    })
    .select("id,staff_member_id,device_name,device_type,serial_number,device_fingerprint,trusted_for_attendance,trusted_at,issued_at,status,note,created_at")
    .single();
