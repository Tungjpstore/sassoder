...
type ApprovalRow = {
  ...
  staff_member_id: string;
  branch_id: string | null;
  request_type: "outside_location" | "attendance_edit" | "overtime" | "shift_override" | "manual_clock_in" | "leave_request" | "shift_swap";
...
const maxOfflineBackfillMs = 7 * 24 * 60 * 60 * 1000;
...
function normalizeCapturedAt(value: string | undefined, source: "gps" | "qr" | "manual" | "offline_sync") {
  ...
  if (source === "offline_sync" && now - capturedAt.getTime() > maxOfflineBackfillMs) {
...
async function resolveBranch({
  ...
  const targetBranchId = branchId || activeAssignments.find((assignment) => assignment.is_primary)?.branch_id;
...
export async function clockInStaffAttendance({
  ...
  const approvalState = gps.valid && !shiftOverrideReason ? "auto_approved" : "pending";
  ...
  const insertResult = await supabase
    ...
    .insert({
      ...
      clock_in_device: input.deviceInfo,
  ...
  await insertActivityLog({
    ...
    severity: approvalState === "pending" || anomaly.score >= 60 ? "warning" : "info",
...
export async function clockOutStaffAttendance({
  ...
  const approvalState = attendance.approval_state === "pending" || !gps.valid || shiftOverrideReason ? "pending" : attendance.approval_state;
  const anomalyFlags = Array.from(new Set([...(attendance.anomaly_flags ?? []), ...anomaly.flags]));
  const anomalyScore = Math.min(100, Math.max(attendance.anomaly_score ?? 0, anomaly.score));
  ...
  const updateResult = await supabase
    ...
    .update({
      ...
      clock_out_device: input.deviceInfo,
      attendance_state: timing.state,
      approval_state: approvalState,
