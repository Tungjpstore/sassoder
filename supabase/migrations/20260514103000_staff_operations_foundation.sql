...
with check (
  ...
  and (staff_user_id = auth.uid() or public.current_user_role() = 'ADMIN')
);
...
with check (
  restaurant_id = public.current_restaurant_id()
  and (requested_by = auth.uid() or public.current_user_role() = 'ADMIN')
);
...
using (
  restaurant_id = public.current_restaurant_id()
  and (user_id is null or user_id = auth.uid() or public.current_user_role() = 'ADMIN')
);
