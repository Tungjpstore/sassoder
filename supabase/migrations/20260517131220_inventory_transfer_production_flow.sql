...
begin
  ...
  if target_action = 'receive' then
    ...
    loop
      ...
      if line_record.received_quantity <= 0 then
        perform public.apply_inventory_movement(
          target_restaurant_id,
          line_record.ingredient_id,
          'transfer_in',
          abs(line_record.dispatched_quantity),
          null,
          'transfer',
          transfer_record.id,
    ...
    set
      ...
      metadata = metadata || jsonb_build_object(
        'lineCount', line_count,
        'totalQuantity', total_quantity,
        'workflow', coalesce(metadata -> 'workflow', '[]'::jsonb) || jsonb_build_array('received')
      ),
      updated_at = now()
    where id = transfer_record.id
      and restaurant_id = target_restaurant_id
...
revoke all on function public.process_branch_transfer(uuid, uuid, text, uuid, text) from public;
grant execute on function public.process_branch_transfer(uuid, uuid, text, uuid, text) to authenticated, service_role;
