-- Phase 2: reserve prepaid order stock without reducing physical on-hand.
-- Reservations are consumed on merchant acceptance and released on cancellation.
...
create table if not exists public.inventory_reservations (
  ...
  constraint inventory_reservations_status_check check (status in ('reserved','consumed','released')),
  constraint inventory_reservations_key_length check (length(trim(allocation_key)) between 1 and 180),
  constraint inventory_reservations_metadata_object check (jsonb_typeof(metadata) = 'object'),
  constraint inventory_reservations_unique_allocation unique (restaurant_id, order_id, allocation_key)
);

create index if not exists inventory_reservations_order_status_idx
  on public.inventory_reservations (restaurant_id, order_id, status);

create index if not exists inventory_reservations_open_stock_idx
  on public.inventory_reservations (restaurant_id, branch_id, ingredient_id, status)
  where status = 'reserved';

create unique index if not exists inventory_movements_order_reserve_unique_idx
  ...
  where source_type = 'order'
    and movement_type in ('reserve', 'release_reserve')
...
begin
  ...
  loop
    ...
    if effective_branch_id is not null and not exists (
      select 1 from public.store_branches where id = effective_branch_id and restaurant_id = target_restaurant_id
    ) then
...
begin
  if coalesce(auth.role(), '') <> 'service_role' then raise exception 'Inventory release requires service role'; end if;
  ...
  loop
    update public.stock_balances
    set reserved_quantity = reserved_quantity - reservation_record.quantity, updated_at = now()
    ...
    if not found then raise exception 'Inventory reservation balance is inconsistent'; end if;
    insert into public.inventory_movements (
      ...
      'Release cancelled prepaid order stock', target_actor_user_id
    ) on conflict do nothing;
    update public.inventory_reservations
    set status = 'released', released_at = now(), actor_user_id = target_actor_user_id
    where id = reservation_record.id;
    released_count := released_count + 1;
  end loop;
  return jsonb_build_object('status', 'released', 'reservationCount', released_count);
end;
$$;
...
create or replace function public.accept_order_with_reserved_inventory(
  target_restaurant_id uuid,
  target_order_id uuid,
  target_actor_user_id uuid default null,
...
begin
  if coalesce(auth.role(), '') <> 'service_role' then raise exception 'Reserved inventory acceptance requires service role'; end if;
  perform public.consume_order_inventory(target_restaurant_id, target_order_id, target_actor_user_id);
  ...
  set status = 'ordering',
      ...
      updated_at = now()
  where id = target_order_id and restaurant_id = target_restaurant_id and status in ('waiting_payment', 'waiting_confirm', 'paid', 'pending', 'ordering')
  returning * into updated_order;
  if not found then raise exception 'Order status changed before reserved inventory acceptance'; end if;
  return updated_order;
end;
$$;

-- Keep reservation release and the existing deduction rollback/order transition
-- in one transaction so a failed cancellation cannot reopen stock prematurely.
create or replace function public.cancel_order_with_inventory_reservation_rollback(
  target_restaurant_id uuid,
  target_order_id uuid,
  target_actor_user_id uuid default null
)
...
begin
  ...
  select * into cancelled_order
  from public.cancel_order_with_inventory_rollback(
    target_restaurant_id,
    target_order_id,
    target_actor_user_id
  );
...
revoke all on function public.release_order_inventory(uuid, uuid, uuid) from public, anon, authenticated;
revoke all on function public.consume_order_inventory(uuid, uuid, uuid) from public, anon, authenticated;
revoke all on function public.accept_order_with_reserved_inventory(uuid, uuid, uuid, timestamptz, text) from public, anon, authenticated;
revoke all on function public.cancel_order_with_inventory_reservation_rollback(uuid, uuid, uuid) from public, anon, authenticated;
grant execute on function public.reserve_order_inventory(uuid, uuid, uuid, jsonb) to service_role;
grant execute on function public.release_order_inventory(uuid, uuid, uuid) to service_role;
grant execute on function public.consume_order_inventory(uuid, uuid, uuid) to service_role;
grant execute on function public.accept_order_with_reserved_inventory(uuid, uuid, uuid, timestamptz, text) to service_role;
grant execute on function public.cancel_order_with_inventory_reservation_rollback(uuid, uuid, uuid) to service_role;
