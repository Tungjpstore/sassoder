...
export async function GET(request: Request) {
  const session = await requireOperationalDashboardApiSession({ feature: "online_ordering" });
  const url = new URL(request.url);
  const size = safeSize(url.searchParams.get("size"));
  const onlineUrl = buildTenantUrl(session.restaurant.slug, "/");
  const qrUrl = new URL("https://api.qrserver.com/v1/create-qr-code/");
  qrUrl.searchParams.set("size", `${size}x${size}`);
  qrUrl.searchParams.set("format", "png");
  qrUrl.searchParams.set("margin", "14");
  qrUrl.searchParams.set("data", onlineUrl);

  const response = await fetch(qrUrl, { cache: "no-store" });
  if (!response.ok || !response.body) {
    return Response.json({ ok: false, error: "Không tạo được mã QR online." }, { status: 502 });
  }
  ...
  if (url.searchParams.get("download") === "1") {
    headers.set("Content-Disposition", `attachment; filename="logivn-online-${session.restaurant.slug}.png"`);
  }
