...
import { listRecentAiRecommendations, updateAiRecommendationStatus } from "@/services/ai-recommendation-service";
...
export async function GET(request: Request) {
  try {
    ...
    return ok(await listRecentAiRecommendations(session.restaurantId, readLimit(request)));
  } catch (error) {
    return fail(error);
  }
}
