  ...
  const metricCards = [
    {
      ...
      detail: latest ? statusLabel(latest.status) : "Đợi hệ thống tạo brief",
      icon: BrainCircuit,
    ...
    },
    {
      ...
      detail: branchInsightsResult.schemaReady ? "AI đang theo dõi chi nhánh" : "Cần bật dữ liệu AI",
      icon: Store,
      ...
      <div className="dashboard-ai-workspace grid gap-3">
        ...
        <AiOperatingLoop
          ...
          stages={[
            ...
            {
              id: "approve",
              value: recommendationsResult.recommendations.length,
              detail: "Đề xuất cần chủ quán duyệt trước khi AI có tay",
              href: "/dashboard/ai-execution",
              tone: recommendationsResult.recommendations.length ? "yellow" : "green"
            },
            {
              id: "act",
            ...
            {
              id: "verify",
              value: branchInsightsResult.insights.filter((insight) => insight.severity === "critical" || insight.severity === "warning").length,
              detail: "Cảnh báo cần kiểm trong ca hôm nay",
              href: "/dashboard/ai-ops",
              tone: branchInsightsResult.insights.some((insight) => insight.severity === "critical") ? "red" : "yellow"
            },
            {
              id: "audit",
              value: latest ? latest.healthScore : "--",
              detail: "Health score từ Morning Brief gần nhất",
              href: "/dashboard/ai-production",
              tone: latest && latest.healthScore < 70 ? "yellow" : "neutral"
            }
          ]}
        />
        <div className="dashboard-ai-toolbar flex flex-wrap items-center justify-between gap-2">
          ...
          <form action={runAiOperationalSweepAction}>
            <button type="submit" className="admin-glow-btn inline-flex min-h-10 items-center justify-center gap-2 rounded-lg px-3 text-sm font-bold">
              <RefreshCw size={16} />
              Chạy AI vận hành ngay
            </button>
          </form>
        </div>
        ...
        <section className="dashboard-panel p-4">
          ...
          <div className="dashboard-ai-card-grid mt-3 grid gap-2 md:grid-cols-2">
            {!branchInsightsResult.schemaReady ? (
              ...
              <div className="grid min-h-24 place-items-center rounded-xl border border-dashed border-[var(--border)] px-4 text-center text-sm font-semibold text-[var(--muted-foreground)] md:col-span-2">
                Chưa có cảnh báo chi nhánh đang mở. Trợ lý vận hành sẽ tự đưa tín hiệu lên khi có kho thiếu, thanh toán treo, giao xa hoặc thiếu nhân sự theo chi nhánh.
              </div>
            )}
          </div>
        </section>
          ...
          <div className="dashboard-panel p-4">
            ...
            {latest ? (
              ...
              <div className="mt-4 grid min-h-36 place-items-center rounded-xl border border-dashed border-[var(--border)] px-4 text-center text-sm font-semibold text-[var(--muted-foreground)]">
                Khi hệ thống tạo brief đầu tiên, inbox sẽ hiển thị tóm tắt, health score và việc cần xử lý tại đây.
              </div>
            )}
          </div>
