export default function AiAutomationLoading() {
  return (
      ...
      <section className="mx-auto grid w-full max-w-[var(--admin-content-max)] gap-3">
        ...
        <section className="dashboard-panel p-4">
          ...
          <div className="mt-3 grid gap-3 xl:grid-cols-2">
            {[0, 1, 2, 3].map((item) => (
              <div key={item} className="h-52 rounded-xl bg-[var(--soft-surface)]" />
            ))}
          </div>
        </section>
        <section className="grid gap-3 xl:grid-cols-[minmax(0,0.72fr)_minmax(0,1.28fr)]">
