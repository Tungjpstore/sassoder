    ...
    >
      ...
      <section className="mt-4 grid gap-4 xl:grid-cols-[1.05fr_0.95fr]">
        <div className="dashboard-panel p-4">
          ...
          <div className="mt-5 h-72 rounded-xl border border-[var(--border)] bg-[var(--soft-surface)] p-5">
      ...
      <section className="mt-4 grid gap-4 xl:grid-cols-[0.9fr_1.1fr]">
        ...
        <div className="dashboard-panel p-4">
          ...
          <div className="mt-6 grid h-64 grid-cols-[repeat(18,minmax(0,1fr))] items-end gap-2 border-b border-l border-[var(--border)] px-4 pb-5">
      ...
      <section className="dashboard-panel mt-4 p-4">
        ...
        <div className="mt-4 overflow-hidden rounded-xl border border-[var(--border)] bg-white">
          ...
          <div className="grid gap-3 border-t border-[var(--border)] bg-[var(--soft-surface)] px-4 py-3 text-sm font-semibold lg:grid-cols-[1.2fr_1fr_0.8fr_0.8fr_1fr]">
            <span>Tổng cộng</span>
            <span className="metric-number">{formatVnd(report.categoryRows.reduce((sum, row) => sum + row.revenue, 0))}</span>
            <span>{report.categoryRows.reduce((sum, row) => sum + row.orderCount, 0)}</span>
            <span>{report.categoryRows.reduce((sum, row) => sum + row.quantity, 0)}</span>
            <span>{formatVnd(report.averageTicket)}</span>
          </div>
        </div>
      </section>
    </AdminShell>
