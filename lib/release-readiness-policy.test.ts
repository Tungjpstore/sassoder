...
function signoff(overrides: Partial<{
  ...
  const matrix = values.includeMatrix
    ? `\n| Persona/Scope | Status | Evidence |\n| --- | --- | --- |\n| owner | Pass | Tenant-A Branch-A authenticated audit |\n| manager | Pass | Tenant-B Branch-B authenticated audit |\n| cashier | Pass | Synthetic payment marker |\n| waiter | Pass | Synthetic order marker |\n| kitchen | Pass | Synthetic kitchen marker |\n| delivery | Pass | Synthetic delivery marker |\n| employee | Pass | Synthetic attendance marker |\n| customer | Pass | Remote and dine-in customer marker |\n| cleanup | Pass | Exact marker cleanup and audit reconciliation |\n| rollback | Pass | Isolated rollback rehearsal |\n`
    : "";
...
test("signoff without the authenticated role, tenant and cleanup matrix is blocked", () => {
  const result = evaluate(signoff({ includeMatrix: false }));

  assert.equal(result.ok, false);
  assert.ok(result.blockers.some((blocker) => blocker.code === "required-evidence-missing"));
});
