...
const staffAvatarLegacyMigration = "20260602143000_staff_avatar_and_open_attendance_recovery.sql";
...
test("late feature migrations create RLS policies with private tenant helpers", () => {
  const offenders = readdirSync(migrationsDir)
    ...
    .filter((fileName) => ![operationsRepairMigration, staffAvatarLegacyMigration].includes(fileName))
