...
const freeItemPromotion: PublicPromotion = {
  ...fixedPromotion,
  id: "promo-free-item",
  code: "GIFTTEA",
...
test("promotion preview applies free item campaigns to matching cart lines", () => {
  assert.equal(
    calculatePublicPromotionDiscount({
      itemSubtotal: 54000,
      items: [{ menuItemId: "tea", quantity: 2, unitPrice: 27000 }],
  ...
  assert.equal(
    promotionEligibilityMessage({
      itemSubtotal: 54000,
      items: [{ menuItemId: "coffee", quantity: 2, unitPrice: 27000 }],
      promotion: freeItemPromotion
    }),
    "Mã sẽ áp dụng khi đơn có món quà tặng hợp lệ."
  );
  assert.equal(promotionDescription(freeItemPromotion), "Tặng 1 món");
});
