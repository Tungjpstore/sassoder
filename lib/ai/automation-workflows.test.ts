...
test("buildAiAutomationWorkflows creates confirm-first branch transfer balancing workflow", () => {
  const workflows = buildAiAutomationWorkflows({
    snapshot: {
      inventory: {
        lowStockCount: 5,
  ...
  const workflow = workflows.find((item) => item.id === "workflow-inventory-branch-transfer-balance");
  assert.ok(workflow);
  assert.equal(workflow.priority, "high");
  assert.equal(workflow.executionMode, "confirm_first");
  assert.ok(workflow.evidence.includes("lowStock=5"));
  assert.ok(workflow?.steps.some((step) => step.status === "needs_confirmation"));
  assert.ok(workflow?.actions.some((action) => action.intent === "inventory_transfers"));
});
