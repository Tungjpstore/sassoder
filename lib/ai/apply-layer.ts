...
export type AiAgentActionKind = "promotion_draft" | "menu_item_draft" | "purchase_order_draft";

export type AiApplyPlan = {
  id: string;
  itemId: string;
  databaseId?: string | null;
  kind: AiExecutionItem["kind"];
  domain: AiExecutionItem["domain"];
  title: string;
...
function ownerCopy(item: AiExecutionItem) {
  ...
  if (item.status === "approved") return "Item đã được duyệt. Nếu có nút tạo nháp, AI sẽ tạo bản nháp an toàn để chủ quán kiểm tra trước khi bật.";
...
function agentActionFor(item: AiExecutionItem): AiApplyPlan["agentAction"] {
  if (item.status !== "approved" || item.kind !== "recommendation" || !item.databaseId) return null;

  if (item.domain === "growth") {
    return {
      kind: "promotion_draft",
      label: "Tạo mã nháp",
      description: "Tạo promotion chưa active, chưa hiện trên menu khách để chủ quán kiểm tra rồi bật sau."
    };
  }

  if (item.domain === "menu") {
    return {
      kind: "menu_item_draft",
      label: "Tạo món nháp",
      description: "Tạo món/combo AI nháp ở trạng thái tạm ẩn, không xuất hiện trên QR menu cho tới khi chủ quán bật bán."
    };
  }
  ...
  if (item.domain === "inventory") {
    return {
      ...
      label: "Tạo PO nháp",
      description: "Gom nguyên liệu dưới ngưỡng thành purchase order cần quản lý kiểm tra trước khi gọi nhà cung cấp."
    };
  }
...
export function buildAiApplyPlan(item: AiExecutionItem): AiApplyPlan {
  return {
    ...
    agentAction: agentActionFor(item),
