...
function requiredEvidenceBlockers(text) {
  const normalized = text.toLowerCase();
  return REQUIRED_EVIDENCE_TOKENS
    .filter((token) => !exactTokenPresent(normalized, token))
    .map((token) => blocker(
      "required-evidence-missing",
      `Release QA is missing required authenticated evidence token "${token}".`
    ));
}
...
export function evaluateReleaseQaSignoff({
  ...
  blockers.push(...requiredEvidenceBlockers(text));
