...
main() {
  ...
  apt-get update
  apt-get upgrade -y
  apt-get install -y curl git unzip htop ufw fail2ban build-essential ca-certificates gnupg lsb-release nginx certbot python3-certbot-nginx logrotate unattended-upgrades postgresql-client
