...
sync_nginx_config() {
  ...
  nginx_has_webhook_routes() {
    local config
    config="$(sudo -n nginx -T 2>/dev/null)"
    printf '%s\n' "$config" | grep -q 'location /webhooks/telegram/' || return 1
    printf '%s\n' "$config" | grep -q 'location /webhooks/platform-telegram/' || return 1
  }
