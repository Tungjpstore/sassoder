...
export async function getAiRecommendationById(restaurantId: string, recommendationId: string) {
  const supabase = createAdminSupabaseClient() as any;
  const result = await supabase
    .from("ai_recommendations")
    .select(
  ...
  if (result.error) {
    if (isMissingAiRecommendationsSchema(result.error)) return { recommendation: null, schemaReady: false };
    throw result.error;
  }

  return {
    recommendation: result.data ? recommendationFromRow(result.data as RecommendationRow) : null,
    schemaReady: true
  };
}
