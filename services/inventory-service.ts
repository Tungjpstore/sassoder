...
export async function listInventoryIngredients(restaurantId: string): Promise<InventoryIngredient[]> {
  ...
  const { data, error } = await db
    .from("ingredients")
    .select("id,restaurant_id,category_id,name,unit,on_hand_quantity,minimum_quantity,reference_unit_cost,is_active,metadata,category:ingredient_categories(name)")
    .eq("restaurant_id", restaurantId)
    .eq("is_active", true)
    .order("name", { ascending: true });
...
export async function getInventoryWarehouseCommandCenter(restaurantId: string): Promise<InventoryWarehouseCommandCenter> {
  ...
  const [
    ...
    db
      .from("branch_transfers")
      .select("id,transfer_number,status,from_location_id,to_location_id,created_at,metadata,lines:branch_transfer_lines(id)")
      .eq("restaurant_id", restaurantId)
      .order("created_at", { ascending: false })
      .limit(8),
    db
  ...
  const transfers = ((transferResult.data ?? []) as BranchTransferRow[]).map((transfer) => {
    ...
    return {
      ...
      lineCount: transfer.lines?.length ?? metadataInteger(metadata, "lineCount"),
      totalQuantity: metadataNumber(metadata, "totalQuantity")
    };
  });
...
export async function deductInventoryForOrder(
  ...
  if (items.length === 0) return { schemaReady: true, movementCount: 0, skippedReason: "no_items" };
