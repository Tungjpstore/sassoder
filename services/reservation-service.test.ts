...
import { createReservationSchema, reservationRescheduleSchema, reservationSetTablesSchema } from "../lib/validators";
...
test("createReservationSchema requires UUID idempotency keys for retry-safe public booking", () => {
  ...
  assert.equal(createReservationSchema.safeParse({ ...validReservation, idempotencyKey: "manual-key" }).success, false);
});
