...
type StoreBranchRow = {
  ...
  accepting_delivery: boolean | null;
...
export async function generateAiBranchOperationInsightsForRestaurant(
  ...
  await ensureDefaultStoreBranch(input.restaurantId);

  const branchesResult = await supabase
    .from("store_branches")
    .select("id,name,address,is_primary,is_active,accepting_delivery,delivery_paused,temporarily_closed,metadata")
  ...
  for (const branch of branches) {
    try {
      ...
      const snapshot: AiBranchOperationSnapshot = {
        ...
        acceptingDelivery: typeof branch.accepting_delivery === "boolean" ? branch.accepting_delivery : metadataBoolean(branch.metadata, "acceptingDelivery"),
        deliveryPaused: typeof branch.delivery_paused === "boolean" ? branch.delivery_paused : metadataBoolean(branch.metadata, "deliveryPaused"),
        temporarilyClosed: typeof branch.temporarily_closed === "boolean" ? branch.temporarily_closed : metadataBoolean(branch.metadata, "temporarilyClosed"),
