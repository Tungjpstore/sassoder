...
4. Fallback low-cost: `Vietmap -> Mapbox -> OSRM -> Nominatim -> Haversine` khi timeout/quota/outage.
...
MAPS_ROUTING_FALLBACKS="goong,vietmap,mapbox,osrm"
...
## Smoke test maps/delivery

Chạy smoke tối thiểu sau khi có server local hoặc staging:

```bash
npm run maps:smoke
```

Smoke tối thiểu kiểm tra `/api/maps/route`, invalid coordinates, `/api/maps/search` và `/api/delivery/fee`. Khi staging có restaurant seed, bật thêm branch routing, delivery quote, Goong primary, Mapbox route path, failed geocoding và invalid quote coordinates:

```bash
MAPS_DELIVERY_SMOKE_BASE_URL="https://staging.example.com" \
MAPS_DELIVERY_SMOKE_RESTAURANT_SLUG="demo-pho" \
MAPS_DELIVERY_SMOKE_REQUIRE_GEOCODE_RESULT="true" \
MAPS_DELIVERY_SMOKE_REQUIRE_NEAREST_STORE="true" \
MAPS_DELIVERY_SMOKE_REQUIRE_DELIVERY_QUOTE="true" \
MAPS_DELIVERY_SMOKE_REQUIRE_GOONG_ROUTE="true" \
MAPS_DELIVERY_SMOKE_REQUIRE_MAPBOX_ROUTE="true" \
MAPS_DELIVERY_SMOKE_REQUIRE_FAILED_GEOCODING="true" \
MAPS_DELIVERY_SMOKE_REQUIRE_INVALID_QUOTE_COORDINATES="true" \
npm run maps:smoke
```

Nếu quán staging không nằm gần cặp tọa độ route mặc định, giữ `ORIGIN/DESTINATION` cho provider route và đặt riêng tọa độ giao hàng gần quán bằng `MAPS_DELIVERY_SMOKE_DELIVERY_LAT/LNG` để strict smoke kiểm tra accepted quote + branch routing đúng dữ liệu seed.

Fail smoke được xem là release blocker nếu lệch phí giao hàng, ETA không thể xảy ra, route hỏng, invalid coordinates không bị chặn hoặc quote chọn khác chi nhánh với `/api/location/nearest-store`.
