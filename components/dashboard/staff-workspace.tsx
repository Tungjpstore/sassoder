...
function permissionSet(role: StaffRole) {
  if (role === "ADMIN") return ["Đơn hàng", "Menu", "Bàn & QR", "Thanh toán", "Nhân viên", "Cài đặt"];
  return ["Đơn hàng", "Bếp", "Bàn & QR", "Thanh toán"];
}
...
export function StaffWorkspace({ users, operations, currentUserId, currentRole, fallbackUser }: StaffWorkspaceProps) {
  ...
  const filteredUsers = useMemo(() => {
    ...
    return sourceUsers.filter((user) => {
      ...
      const matchesKeyword = !keyword || user.email.toLowerCase().includes(keyword) || user.email.split("@")[0].toLowerCase().includes(keyword);
  ...
  const stats = [
    { label: "Tổng tài khoản", value: sourceUsers.length, meta: `${adminCount} quản lý`, icon: Users },
    { label: "Nhân viên", value: staffCount, meta: "Tài khoản STAFF", icon: UserRound },
        ...
        <div className="dashboard-panel p-4">
          <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm font-medium text-[var(--muted-foreground)]">Màn chính chỉ hiển thị danh sách và vai trò. Thêm mới hoặc chỉnh quyền sẽ mở ở drawer riêng để tránh chiếm diện tích.</p>
            </div>
              ...
              {filteredUsers.map((user, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span><Badge tone={user.role === "ADMIN" ? "green" : "neutral"}>{roleLabel(user.role)}</Badge></span>
                    <span className="truncate text-sm font-semibold text-[var(--muted-foreground)]">{user.email}</span>
                    <span><Badge tone="green">Đang hoạt động</Badge></span>
                  </button>
                );
              })}
            </div>
              ...
              <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-5">
                {panelMode === "create" && (
                  <form action={createStaffAction} className="grid gap-4 rounded-xl border border-[var(--border)] bg-[var(--soft-surface)] p-4">
                    ...
                    <Button disabled={!canManage} className="shadow-none hover:shadow-none">
                      <UserPlus size={16} />
                      Tạo tài khoản
                    </Button>
                  </form>
                ...
                {panelMode === "staff" && selectedUser && (
                  <div className="grid gap-4">
                    ...
                    <div className="rounded-xl border border-[var(--border)] bg-[var(--surface)] p-4">
                      ...
                      <div className="mt-3 flex flex-wrap gap-2">
                        {permissionSet(selectedUser.role).map((permission) => (
                          <Badge key={permission} tone="green">{permission}</Badge>
                        ))}
                      </div>
                    </div>
                    ...
                    <form key={selectedUser.id} action={updateStaffRoleAction} className="grid gap-3 rounded-xl border border-[var(--border)] bg-[var(--surface)] p-4">
                      ...
                      <Button disabled={!canManage || selectedUser.id === currentUserId} className="shadow-none hover:shadow-none">
                        <ShieldCheck size={16} />
                        Lưu quyền truy cập
                      </Button>
                    </form>
