  ...
  return createPortal(
    <div className="dashboard-drawer-root fixed inset-0 isolate z-[var(--z-dashboard-drawer)] overflow-hidden overscroll-contain">
      ...
      <aside
        ...
        tabIndex={-1}
        className={cn("drawer-panel absolute inset-y-0 right-0 z-10 flex h-dvh max-h-dvh w-full flex-col border-l border-[var(--border)] bg-[var(--surface)]", widthMap[width])}
      >
        <div className="flex shrink-0 items-start justify-between gap-3 border-b border-[var(--border)] px-4 py-3 sm:px-5 sm:py-4">
          <div className="min-w-0">
            ...
            <h2 id={titleId} className="dashboard-section-title mt-1 truncate">
              {title}
            </h2>
        ...
        <div
          data-dashboard-drawer-content
          className={cn(
            "min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 py-4 sm:px-5",
            footer ? "pb-[calc(1rem+env(safe-area-inset-bottom))]" : "pb-[calc(5rem+env(safe-area-inset-bottom))]",
            contentClassName
          )}
        >
          {children}
