...
const nav = [
  ...
  { label: "Năng lực", href: "#capabilities", id: "capabilities" },
  { label: "Cách dùng", href: "#how", id: "how" },
  { label: "Bảng giá", href: "#pricing", id: "pricing" },
  { label: "Câu hỏi", href: "#faq", id: "faq" }
];

export function LandingHeaderV3({ logoUrl, label }: { logoUrl: string; label: string }) {
  ...
  return (
    <header
      className={[
        ...
        scrolled ? "border-b border-[var(--line)] v3-glass" : "border-b border-transparent"
      ].join(" ")}
    >
      <Container className="flex h-16 items-center justify-between gap-4">
          ...
          {nav.map((n) => {
            ...
            return (
              <a
                ...
                className={[
                  "relative inline-flex h-10 items-center rounded-[var(--r-pill)] px-3.5 text-[length:var(--fs-sm)] font-semibold transition",
                  isActive ? "text-[var(--jade)]" : "text-[var(--text-muted)] hover:bg-[var(--primary-soft)] hover:text-[var(--jade)]"
                ].join(" ")}
              >
                {n.label}
                {isActive ? <span className="absolute inset-x-3.5 -bottom-px h-0.5 rounded-full bg-[var(--jade)]" /> : null}
              </a>
            );
          })}
        </nav>
        ...
        <div className="hidden items-center gap-2 lg:flex">
          ...
          <Magnetic strength={0.25}>
            <ButtonV2 as="link" href="/dashboard/register?plan=pro" variant="primary" size="md" className="v3-shimmer">
              Dùng thử
              <ArrowRight size={14} />
            </ButtonV2>
          </Magnetic>
...
export function LandingFooterV3({ logoUrl, label }: { logoUrl: string; label: string }) {
  return (
    <footer className="relative border-t border-[var(--line)] bg-[var(--surface)]/85 backdrop-blur-sm">
          ...
          {footerColumns.map((col) => (
            <nav key={col.title} aria-label={col.title} className="flex flex-col gap-[var(--s-4)]">
              ...
              <ul className="flex flex-col gap-[var(--s-3)]">
                {col.links.map((l) => (
                  <li key={l.label}>
                    <Link href={l.href} className="text-[length:var(--fs-sm)] text-[var(--text-muted)] transition-colors hover:text-[var(--jade)]">
                      {l.label}
                    </Link>
                  </li>
                ))}
